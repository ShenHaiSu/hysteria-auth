  ...
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <AppStatusBadge
          v-if="node"
          :type="isOnline(node) ? 'online' : 'offline'"
        />
      </div>
    ...
    <div v-else-if="node" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1 bg-[var(--bg-elevated)] rounded-md border border-[var(--border-light)] shadow-sm p-5 space-y-4">
        ...
        <div class="space-y-3">
          ...
          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{ t('nodes.table.columns.name') }}</span>
            <p class="text-sm text-[var(--text-primary)] font-medium">{{ node.name }}</p>
          </div>

          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{ t('nodes.table.columns.ipAddress') }}</span>
            <p class="text-sm text-[var(--text-primary)] font-mono">{{ node.ipAddress ?? '-' }}</p>
          </div>

          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{ t('nodes.table.columns.port') }}</span>
            <p class="text-sm text-[var(--text-primary)]">{{ node.port }}</p>
          </div>

          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{ t('nodes.table.columns.location') }}</span>
            <p class="text-sm text-[var(--text-primary)]">{{ node.location ?? '-' }}</p>
          </div>

          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{ t('nodes.table.columns.provisionStatus') }}</span>
            <div class="mt-1">
              <AppStatusBadge
          ...
          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{ t('nodes.table.columns.lastHeartbeat') }}</span>
            <p class="text-sm text-[var(--text-primary)]">{{ formatHeartbeat(node.lastHeartbeat) }}</p>
          </div>

          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{ t('nodes.table.columns.createdAt') }}</span>
        ...
        <div
          v-if="canEdit"
          class="mt-4 pt-4 border-t border-[var(--border-light)]"
        >
          ...
          <div class="space-y-2">
            <div>
              <span class="text-xs text-[var(--text-muted)]">{{ t('nodes.detail.secretVersion') }}</span>
              <p class="text-sm text-[var(--text-primary)] font-mono">{{ node.secretVersion }}</p>
            </div>
            <div v-if="node.trafficStatsSecret">
              <span class="text-xs text-[var(--text-muted)]">{{ t('nodes.form.fields.trafficStatsPort.label') }} Secret</span>
      ...
      <div class="lg:col-span-2 bg-[var(--bg-elevated)] rounded-md border border-[var(--border-light)] shadow-sm p-5 space-y-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div class="bg-[var(--bg-secondary)] rounded-md p-3">
            <p class="text-xs text-[var(--text-muted)] mb-1">{{ t('nodes.status.activeConnections') }}</p>
            <p class="text-lg font-bold text-[var(--text-primary)]">
        ...
        <!-- 图表占位 (Phase 5 补充) -->
        <div class="bg-[var(--bg-secondary)] rounded-md p-6 flex items-center justify-center text-sm text-[var(--text-muted)]" style="min-height: 280px">
          <div class="text-center">
            <i class="pi pi-chart-bar text-3xl mb-2 block text-[var(--text-muted)]" />
            <p>{{ t('nodes.status.cpu') }} / {{ t('nodes.status.memory') }} / {{ t('nodes.table.columns.name') }}</p>
            ...
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-[var(--border-light)]">
                  <th class="text-left py-2 px-3 text-xs text-[var(--text-muted)] font-medium">{{ t('nodes.status.cpu') }}</th>
                  <th class="text-left py-2 px-3 text-xs text-[var(--text-muted)] font-medium">{{ t('nodes.status.memory') }}</th>
                  <th class="text-left py-2 px-3 text-xs text-[var(--text-muted)] font-medium">{{ t('nodes.status.networkIn') }}</th>
                  <th class="text-left py-2 px-3 text-xs text-[var(--text-muted)] font-medium">{{ t('nodes.status.networkOut') }}</th>
                  <th class="text-left py-2 px-3 text-xs text-[var(--text-muted)] font-medium">{{ t('nodes.status.activeConnections') }}</th>
                  <th class="text-right py-2 px-3 text-xs text-[var(--text-muted)] font-medium">时间</th>
                </tr>
              </thead>
              <tbody>
                <tr
                ...
                >
                  <td class="py-2 px-3 text-[var(--text-primary)]">{{ formatPercent(record.cpuUsagePercent) }}</td>
                  <td class="py-2 px-3 text-[var(--text-primary)]">{{ formatPercent(record.memoryUsagePercent) }}</td>
                  <td class="py-2 px-3 text-[var(--text-primary)]">{{ formatMbps(record.networkInMbps) }}</td>
                  <td class="py-2 px-3 text-[var(--text-primary)]">{{ formatMbps(record.networkOutMbps) }}</td>
                  <td class="py-2 px-3 text-[var(--text-primary)]">{{ record.activeConnections }}</td>
