<template>
  <div class="p-6 space-y-6">
    ...
    <div v-else-if="user" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        <div class="space-y-3">
          ...
          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{
              t('users.table.columns.isActive')
            }}</span>
            <div class="mt-1">
          ...
          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{
              t('users.table.columns.expiresAt')
            }}</span>
          ...
          <div>
            <span class="text-xs text-[var(--text-muted)] uppercase tracking-wide">{{
              t('users.table.columns.allowedNodes')
            }}</span>
      ...
      >
        <!-- 操作栏：标题 + 导出/周期选择（响应式：移动端上下排列，桌面端同行） -->
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div class="bg-[var(--bg-secondary)] rounded-md p-4">
            <p class="text-xs text-[var(--text-muted)] mb-1">
              {{ t('users.traffic.usageRate') }}
            </p>
            <p
              class="text-2xl font-bold"
              :class="
                user.totalTrafficBytes > 0 && user.usedTrafficBytes / user.totalTrafficBytes > 0.9
    ...
    <!-- 错误状态 -->
    <AppError v-if="!usersStore.isLoading && !user" @retry="loadUser" />

    <!-- 编辑对话框 -->
    <UserFormDialog v-if="user" v-model:visible="showEditDialog" :user="user" @saved="loadUser" />
  </div>
</template>
...
const periodOptions = computed(() => [
  { label: t('users.traffic.period.day'), value: 'day' as const },
  { label: t('users.traffic.period.week'), value: 'week' as const },
  { label: t('users.traffic.period.month'), value: 'month' as const },
  { label: t('users.traffic.period.all'), value: 'all' as const },
])

/** 响应式图表高度：移动端 260px，桌面端 (≥lg) 360px */
const chartHeight = computed(() => {
  return window.innerWidth >= 1024 ? '360px' : '260px'
})
...
const trafficChartOption = computed<EChartsOption | null>(() => {
  ...
  const inData = stats.dataPoints.map(
    (p: TrafficDataPoint) => +(p.bytesIn / (1024 * 1024)).toFixed(2),
  )
  const outData = stats.dataPoints.map(
    (p: TrafficDataPoint) => +(p.bytesOut / (1024 * 1024)).toFixed(2),
  )
...
async function handleResetTraffic() {
  if (!user.value) return
  const confirmed = await confirm.danger(t('users.toast.resetConfirm'), t('common.confirm.title'))
  if (confirmed) {
    try {
