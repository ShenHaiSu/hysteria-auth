...
- 权限：公开（无令牌）；成功后返回令牌
- 作用：用户登录并获取会话令牌与基础信息
- 原理：根据用户名与 MD5 口令匹配激活用户，签发 HS256 令牌
- 请求体结构：
...
- 路径：`/api/logout`
- 方法：`POST`
- 权限：公开（后端不校验令牌，直接返回成功）
- 作用：用户退出登录
- 原理：由于使用 JWT 无状态认证，后端仅返回成功标识。前端收到响应后应清除本地存储的 Token。
- 请求体结构：无
- 响应体结构：
...
- 原理：校验来源 IP 命中启用节点，再校验用户代理密码，返回标识
- 请求体结构：
...
- 作用：查询代理节点列表
- 原理：按筛选条件拼接 SQL；普通用户默认附加 is_active=1
- 请求体结构：无；支持查询参数：
