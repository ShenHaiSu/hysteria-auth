...
| 2   | [`system-architecture.md`](system-architecture.md)                         | **系统架构设计** — 整体架构图、主服务器架构、边缘节点架构                                 |
...
| 7   | [`project-structure-configuration.md`](project-structure-configuration.md) | **项目结构与配置说明** — 目录结构、主服务器/Agent 完整配置项                              |
| 8   | [`deployment.md`](deployment.md)                                           | **部署方案与数据备份** — 主服务器/边缘节点部署脚本、Nginx 反向代理、备份恢复策略          |
| 9   | [`traffic-statistics.md`](traffic-statistics.md)                           | **流量统计与会话管理** — 方案选型、采集流程、流量检查、会话生命周期、并发控制             |
| 10  | [`resilience-monitoring.md`](resilience-monitoring.md)                     | **异常恢复、监控与测试** — 网络异常降级、日志体系、SLO 指标、测试策略                     |
| 11  | [`scalability.md`](scalability.md)                                         | **扩展性考虑** — 数据库迁移、水平扩展、未来功能规划                                       |
| 12  | [`appendix.md`](appendix.md)                                               | **附录** — Hysteria 协议参考、流量统计 API 参考、常用命令、文档修订历史                   |
...
```
概述 + 架构  ──→  数据库  ──→  API  ──→  边缘节点  ──→  流量统计
overview.md      database    api        edge-node     traffic-statistics
system-arch.md   design.md   design.md  design.md     .md

                                       ↓
安全 ← 配置 ← 部署 ← 监控测试 ← 扩展性 ← 附录
security  project  deployment  resilience  scalability  appendix
.md       -struct  .md         -monitoring .md          .md
          ure.md               .md
```
...
| **前端/管理端开发** | api-design（§4.2~4.4）→ security-design（§6.4~6.5）                                                         |
| **运维/DevOps**     | deployment → security-design → resilience-monitoring（§13）→ appendix                                       |
...
- 代码块标注语言类型以启用语法高亮
- 配置示例使用 JSON/YAML/Bash/C#/Nginx 等标注
